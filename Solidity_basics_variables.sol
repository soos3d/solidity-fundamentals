...
pragma solidity >= 0.6.0 <0.9.0;            // Always specify what solity versions are used at the top
                                            // this is using the compiler between version 0.6.0 and 0.9.0
...
function stateVariable() public pure returns(uint){
    uint value = 10;            // state variables can be modified and interacted with inside a function
    value = value + 5;
    return value;
}
